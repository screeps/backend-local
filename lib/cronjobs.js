var express = require('express'),
    ...
    strongholds = require('./common/strongholds');
...
config.cronjobs = {
    ...
    genStrongholds: [5*60, genStrongholds]
};
            ...
            return db['rooms.objects'].find({$and: [{room: room._id}, {type: {$in: ['source','controller','creep']}}]})
                .then(objects => {
                    ...
                    return q.all([
                        ...
                        db['rooms.objects'].count({$and: [{type: 'invaderCore'}, {room: {$regex: sectorRegex}}]})
                    ])
...
async function genStrongholds() {
    ...
    for(let s of sectorsWithoutCores) {
        await strongholds.selectRoom(s, {db, gameTime});
    }
};
