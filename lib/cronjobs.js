...
async function genDeposits(){
    ...
    const [centralRooms, deposits] = await q.all([
        db['rooms'].find({_id: {$regex: '^[WE]\d*5[NS]\d*5$'}, status: {$ne: 'out of borders'}}),
        db['rooms.objects'].find({type: 'deposit'})
    ]);

    const promises = [];

    for(let center of centralRooms) {
        ...
        const [objects, terrain] = await q.all([db['rooms.objects'].find({room: room._id}), db['rooms.terrain'].findOne({room: room._id})]);
