...
router.post('/console', auth.tokenAuth, jsonResponse((request) => {
    var expression = JSON.stringify(request.body.expression);
    if (expression === undefined) {
        return q.reject('invalid expression');
    }
...
router.get('/find', jsonResponse((request) => {
    ...
    return db.users.findOne(query, {username: true, badge: true, gcl: true, power: true, 'steam.id': true, 'steam.steamProfileLinkHidden': true})
        .then((user) => {
            var result = _.pick(user, ['_id','username','badge','gcl','power']);
            if(user.steam && !user.steam.steamProfileLinkHidden) {
...
router.get('/money-history', auth.tokenAuth, jsonResponse((request) => {
    ...
    return db['users.money'].findEx({user: request.user._id}, {sort: {date: -1}, offset: page*pageSize, limit: pageSize+1})
        .then(data => {
...
router.get('/badge-svg', (request, response, next) => {
    var username = request.query.username.toLowerCase() + (request.query.border == 1);
    q.when()
        .then(() => {
                ...
                return db.users.findOne({usernameLower: request.query.username.toLowerCase()})
                    .then(user => {
                        badgeCache[username] = {
                            time: Date.now(),
                            svg: user.badge ?
                                badge.getBadgeSvg(user.badge, request.query.border == 1) :
...
router.post('/set-steam-visible', auth.tokenAuth, jsonResponse(request => {
    return db['users'].update(
        {_id: request.user._id},
        {$merge: {steam: {steamProfileLinkHidden: request.body.visible ? 0 : 1}}}
    )
}));
