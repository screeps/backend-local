...
router.post('/upgrade', auth.tokenAuth, jsonResponse((request) => {
    console.log('upgrade power creep');
    function powersAreValid(powers) {
        const finalLevel = _.sum(powers);

        const recreated = _.zipObject(Object.keys(C.POWER_INFO), _.times(_.size(C.POWER_INFO), () => ({ level: 0 })));
        ...
        while(level < finalLevel) {
            const power = _.first(
                _.filter(
                    Object.keys(powers),
                    k => (recreated[k].level < 5) &&
                        (recreated[k].level < powers[k]) &&
                        (C.POWER_INFO[k].level[recreated[k].level] <= level)));
            if(!power) {
                return false;
            }

            recreated[power].level++;
            level = _.sum(recreated, 'level');
        }
    ...
    return db['users.power_creeps'].find({user: request.user._id})
        .then(userPowerCreeps => {
            ...
            if(!powersAreValid(request.body.powers)) {
                return q.reject('powers set is not valid');
            }
