...
router.post('/map-stats', auth.tokenAuth, jsonResponse((request) => {
    ...
    .then((data) => {
        ...
        return db['rooms.objects'].find({$and: [{room: {$in: request.body.rooms}}, {type: 'controller'}]});
    })
    .then((data) => {
        data.forEach((i) => {
            if (i.user) {
...
router.post('/place-spawn', auth.tokenAuth, jsonResponse((request) => {
    ...
    // OK
    .then(() => db['rooms.objects'].removeWhere({$and: [{room: request.body.room}, {user: {$ne: null}}, {type: {$in: _.keys(C.CONSTRUCTION_COST).concat(['creep'])}}]}))
...
router.get('/world-size', jsonResponse(request => {
    return {width: C.WORLD_WIDTH, height: C.WORLD_HEIGHT};
}));
