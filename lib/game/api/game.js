var express = require('express'),
    ...
    system = common.configManager.config.common.system,
    worldSize;
...
router.post('/map-stats', auth.tokenAuth, jsonResponse((request) => {
    ...
    .then((data) => {
        data.forEach((i) => {
            stats[i._id] = {status: i.status, novice: i.novice, respawnArea: i.respawnArea, openTime: i.openTime};
        });
...
router.post('/place-spawn', auth.tokenAuth, jsonResponse((request) => {
    ...
    .then(() => db['rooms.objects'].update({$and: [{room: request.body.room}, {type: 'controller'}, {level: 0}]},
        {$set: {user: ""+request.user._id, level: 1, progress: 0, downgradeTime: null, safeMode: gameTime + C.SAFE_MODE_DURATION }}))
    ...
    .then(() => db.rooms.update({_id: request.body.room}, {$set: {invaderGoal: 1000000}}))
    .then(() => utils.activateRoom(request.body.room))
...
/**
 * rooms terrain by shard
 */
router.post('/rooms', jsonResponse(async (request) => {
    const rooms = request.body.rooms;
    const data = await db['rooms.terrain'].find({room: {$in: rooms}});
...
router.post('/add-object-intent', auth.tokenAuth, jsonResponse(async request => {
    ...
    if(request.body.name == 'activateSafeMode') {
        ...
        if(count > 0) {
            return q.reject('safe mode active already');
        }
    }

    const intent = system.sanitizeUserIntents({[request.body.name]: request.body.intent}, config.backend.customIntentTypes);

    await db['rooms.intents'].update({room: request.body.room},
        {$merge: {users: {[request.user._id.toString()]: {objectsManual: {[request.body._id]: intent}}}}},
        {upsert: true});
    await utils.activateRoom(request.body.room);
    return q.when();
}));
...
router.post('/create-construction', auth.tokenAuth, jsonResponse((request) => {
    ...
    .then((data) => {
        return utils.activateRoom(request.body.room)
        .then(() => ({_id: data._id}));
    });
}));
...
router.post('/add-global-intent', auth.tokenAuth, jsonResponse((request) => {
    return db['users.intents'].insert({
        ...
        intents: system.sanitizeUserIntents(
            {[request.body.name]: [request.body.intent]},
            config.backend.customIntentTypes)});
}));
