    ...
    function spawn(botAiName, roomName, opts) {
        ...
        try {
            ...
            return db['rooms.objects'].findOne({$and: [{room: roomName}, {type: 'controller'}]})
                ...
                .then(terrainItem => {
                    var x = opts.x || Math.floor(3 + Math.random()*44);
                    var y = opts.y || Math.floor(3 + Math.random()*44);
                    while(common.checkTerrain(terrainItem.terrain, x, y, C.TERRAIN_MASK_WALL)) {
                        x = Math.floor(3 + Math.random()*44);
                        y = Math.floor(3 + Math.random()*44);
                    }
                ...
                .then(() => db.rooms.update({_id: roomName}, {$set: {invaderGoal: 1000000}}))
                .then(() => utils.activateRoom(roomName))
