...
const q = require('q');
const strongholds = require('../common/strongholds');
...
exports.spawn = utils.withHelp([
    ...
    * user - id of user which stronghold structures should belong to, default is "2" (Invader)`,
    function spawn(roomName, opts) {
        opts = opts || {};
        try{
            const gameTime = common.getGametime();
            const scope = { db, gameTime };
            return strongholds.spawnStronghold(roomName, opts, scope);
        }
        catch(e) {
            return q.reject(e);
        }
