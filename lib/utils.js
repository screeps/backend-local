...
exports.respawnUser = function(userId) {
    ...
    return db.users.update({_id: userId}, {$set: {lastRespawnDate: Date.now()}})

    .then(() => db['rooms.objects'].find({$and: [{user: ""+userId}, {energy: {$gt: 0}}]}))
    .then((data) => {
        if(!data.length) {
            return q.when();
        }
        data = _.map(data, (i) => new Object({type: 'energy', room: i.room, x: i.x, y: i.y, energy: i.energy}));
        return db['rooms.objects'].insert(data);
    })
    .then(() => db['rooms.objects'].removeWhere({$and: [
        {user: ""+userId},
        {type: {$ne: 'controller'}}]}))
